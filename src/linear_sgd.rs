//! Linear SGD - Alternative path-guided stochastic gradient descent
//!
//! This is a simpler SGD variant compared to `path_sgd.rs`. It uses a more
//! straightforward approach while still maintaining ODGI-compatible behavior.
...
impl LinearSGD {
    ...
    pub fn run(&self) -> HashMap<usize, f64> {
        ...
        let delta_max = Arc::new(AtomicU64::new(f64_to_u64(0.0)));
        ...
        let eta = Arc::new(AtomicU64::new(f64_to_u64(eta_max)));
        ...
        for tid in 0..self.nthreads {
            ...
            let handle = thread::spawn(move || {
                ...
                let step_dist = Uniform::new(0, all_steps.len()).unwrap();
                ...
                while work_todo.load(Ordering::Relaxed) {
                    ...
                    let step_b_idx = if cooling.load(Ordering::Relaxed) || flip_dist.sample(&mut rng) == 1 {
                        ...
                        let uniform = Uniform::new(0, path.steps.len()).unwrap();
    ...
    fn initialize_atomic_positions(&self) -> HashMap<usize, AtomicU64> {
        ...
        for node_id in node_ids {
            positions.insert(node_id, AtomicU64::new(f64_to_u64(current_pos)));
